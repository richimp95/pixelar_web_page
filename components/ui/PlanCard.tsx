...
type Props = {
  ...
  selected?: boolean;
  onSelect?: () => void;
  /** Cuando true (toggle Anual), el precio no cambia; solo el color de los
   * textos clave (título, precio, valores) pasa a cobre para marcar el modo,
   * sin invertir el fondo de la tarjeta. */
  isYearly?: boolean;
  className?: string;
};
...
export function PlanCard({
  ...
  return (
    ...
    >
      ...
      {/* Encabezado: en Anual, el título y el precio pasan a cobre (única
          diferencia de color; el fondo de la tarjeta no cambia). */}
      <h3
        className={cn(
      ...
      <div className="relative mt-5 flex items-baseline gap-1">
        ...
        <span className={cn("text-sm", isYearly ? "text-accent/80" : "text-slate-300")}>{shownPeriod}</span>
      </div>
      ...
      <div className="relative mt-6 border-t border-white/10 pt-6">
        <p className={cn("mb-3 text-sm font-medium", isYearly ? "text-accent/90" : "text-slate-200")}>Incluye:</p>
          ...
          {features.map((f) => {
            ...
            return (
              ...
              >
                ...
                <span className={cn("ml-3 shrink-0 text-sm font-semibold", isYearly ? "text-accent" : "text-slate-100")}>
                  {f.detail}
                </span>
              </div>
            );
      ...
      {note ? (
        <p
          className={cn(
            ...
            isYearly ? "text-accent/80" : "text-slate-300"
          )}
        >
          {note}
        </p>
